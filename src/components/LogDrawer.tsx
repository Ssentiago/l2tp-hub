...
function levelColor(msg: string): string {
  ...
  if (m.includes("error") || m.includes("err:") || m.includes("failed"))
    return "#f28b82";
  if (m.includes("warn") || m.includes("timeout")) return "#fdd663";
  if (m.includes("connect") || m.includes("ok") || m.includes("success"))
    return "#81c995";
  return "#8ab4f8";
}
...
export function LogDrawer({ open, onClose }: Props) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setEntries((prev) => {
        const next =
          prev.length + buf.length > MAX_ENTRIES
  ...
  useEffect(() => {
    if (paused || !open || userScrolledUp.current || entries.length === 0)
      return;
  ...
  return (
    ...
    >
      ...
      <Box sx={{ px: 2, py: 1, flexShrink: 0 }}>
        ...
        >
          Хранение: 45 мин · макс {MAX_ENTRIES} записей · батч{" "}
          {FLUSH_INTERVAL_MS}ms{paused && " · ⏸ пауза"}
        </Typography>
      </Box>
    </Drawer>
  );
