...
export const api = {
  ...
  connectVpn: (id: string) => invoke<void>("connect_vpn", { id }),

  disconnectVpn: (id: string) => invoke<void>("disconnect_vpn", { id }),

  getVpnStatus: (id: string) => invoke<VpnStatus>("get_vpn_status", { id }),

  authenticateSudo: (password: string) =>
    invoke<void>("authenticate_sudo", { password }),

  checkSudoSession: () => invoke<boolean>("check_sudo_session"),
